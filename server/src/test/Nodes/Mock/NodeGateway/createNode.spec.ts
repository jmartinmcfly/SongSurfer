...
import { NodeCollectionConnection, NodeGateway } from '../../../../nodes'
...
describe('Unit Test: Create Node', () => {
  ...
  test('inserts valid nested node', async () => {
    ...
    const getParentResp = await nodeCollectionConnection.findNodeById('1')
    expect(
      getParentResp.payload.filePath.children.includes(nestedNode.nodeId)
    ).toBeTruthy()
    const getNestedResp = await nodeCollectionConnection.findNodeById('2')
